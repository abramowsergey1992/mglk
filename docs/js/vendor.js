		...
		_proto5.kill = function kill(revert, matchMedia) {
			...
			if (revert) {
				...
				this.data.forEach(function (t) {
					if (t.data === "isFlip") {
						t.revert();
						t.getChildren(true, true, false).forEach(function (
							tween
						) {
			...
			if (endValues.length) {
				while ((result = _numWithUnitExp.exec(end))) {
					...
					if (
						...
						endValue.charAt(1) === "=" &&
							(endValue =
								_parseRelative(startNum, endValue) + startUnit);
